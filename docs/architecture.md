...
## V4.1 ocean ownership

`OceanCatalog` validates shallow/deep swimming multipliers, the swim stamina drain and the rowboat's speed multiplier, 16-boat global cap and two-tile deploy range from `data/ocean.json` against the canonical item catalog. It contains no runtime boat or swimming state.

`SurvivalState` schema 2 owns the bounded oxygen attribute. Deep surface water drains oxygen, an empty tank applies the data-driven drowning effect exactly like starvation, surfacing clears it and oxygen recovers; `world_sandbox.gd` composes the immutable `in_deep_water` environment flag from the active layer, base terrain and boarding state. `PlayerCharacter` receives a composed terrain speed multiplier and swim-drain value and never queries terrain itself.

`BoatState` alone owns deployed boats as tile-qualified `surface:<x>:<y>` records validated against the ocean catalog. Deployment scans the facing water tiles, consumes exactly one boat item and rejects occupied tiles, ocean resources and over-cap deployment. Boarding is session state: the boarded boat follows the player's water tile at boat speed without oxygen drain, dry land moors the boat at its last water tile through an atomic relocate, and layer changes clear boarding without touching records. `BoatLayer` draws the small fleet and owns no rules. Boats persist inside their owning surface chunk difference as `deployed_boats`; formats 25 and earlier import none.

`TerrainGenerator` classifies the island biome from a dedicated island-mask noise domain plus a genuine-islet rule for low-elevation land with at most two land neighbors; the rule reclassifies existing land tiles only, so terrain bytes, building legality and structure anchors are unchanged while generation version 6 documents the biome and water-resource byte change.

`ResourceGenerator` evaluates ocean resources on a separate stable `resource-water-cell` hash channel: water candidates rank catalog definitions by allowed terrain and biome weights, compete for spacing among all candidates, and land cells keep their exact previous channel. `EnemySpawnPlanner` accepts aquatic definitions only on allowed water tiles without resource overlap and rejects them on land; land candidates keep the dry-land rule. `StructurePlanner` plans shipwrecks and sea ruins on a separate water region roll and validates every transformed cell against water terrain. The fourth regional boss anchors only in open water through the same pure planner probes.
