...
Survival state stores hunger and wetness in `0..100`, body temperature in the catalog's supported range, one unique record per known active effect and exactly the configured exposure keys. Durations, tick offsets and exposure values are finite and bounded; unknown effects, duplicate IDs or non-finite numbers reject the document. Environment inputs and the survival-enabled setting are runtime configuration and are not duplicated in the save. Formats 2–17 initialize neutral hunger, normal temperature, zero wetness/exposure and no effects.
...
Loading validates readable JSON objects, current save version 25 or migratable versions 2–24, supported generation version, required metadata, matching layers, weather, player attributes, inventory, crafting, combat, graves, milestones, dungeon/exploration/regional-Boss/NPC/relationship/quest/faction/world-event/region-progression/world-choice/survival/equipment/homestead invariants and all layer-difference arrays. Current-format worlds must use generation 5. Resource/chest keys plus building, farming and animal coordinates require signed integers; every placement, plot and interacted animal must belong to the file's declared surface chunk, satisfy its complete schema and not overlap another mutable overlay. Processor-only fields additionally require a processor definition and bounded fuel; machine-only fields require a canonical automation definition, bounded time/cycles/fuel and valid recipe/filter IDs. Homestead metadata must match every physical beacon ID and tile exactly. Dungeon keys require a valid entrance-derived scope. Parse failures include filename, parser line and message. Continue remains on the menu and displays `SaveManager.last_error`; it never silently starts a new world over damaged data.

## Format-2 through format-24 migration
...
All paths update metadata and player state in memory to save format 25 and transactionally commit format 25 on the next save. Migration never serializes generated terrain or recreates unmodified chunk files.
